...
contract RaiseDAORescure is ReentrancyGuard,Ownable {
  ...
  mapping (address => UserInfo) public userInfo;
    ...
    // address => amount
  mapping (address => UserInfo) public userInfoSyncView;
  ...
  function setRaiseDao(address _dao) public onlyOwner{
      dao = RaiseDAO(_dao);
  }
  ...
  function getUserCurrentRlease(address addr) public view returns(uint256) {
    ...
    require (user.synced,'must sync data first');
    if(user.amount==0){
      return 0;
    }else{
  ...
  function syncUser(address account) internal  {

      UserInfo storage user = userInfo[account];
      if(user.synced || user.amount > 0 || user.claimed ) {
        return;
      }

      (uint256 amount ,bool claimed ,uint256 offeringTokenAmount ,uint256 amountDept ,uint256 refundingTokenAmount ,bool refundClaimed)= dao.userInfo(account);

      user.amount = amount;
      user.claimed = claimed;
      user.offeringTokenAmount = dao.getOfferingAmount(account);
      user.amountDept = amountDept;
      user.refundingTokenAmount =  dao.getRefundingAmount(account);
      user.refundClaimed = refundClaimed;
      user.synced = true;
  }


  function syncUserTest(address account) public onlyOwner  {
      UserInfo storage user = userInfoSyncView[account];
      if(user.synced || user.amount > 0 || user.claimed ) {
        return;
      }
   ...
   function syncUserV(address account) public view returns(uint256 amount ,bool claimed ,uint256 offeringTokenAmount ,uint256 amountDept ,uint256 refundingTokenAmount ,bool refundClaimed)  {
      ( amount , claimed , offeringTokenAmount , amountDept , refundingTokenAmount , refundClaimed)= dao.userInfo(account);
  ...
  uint256  public harvestOfferingBlock;
  uint256  public harvestRefundingBlock;
  
  function setHavestBlock(uint256 _harvestOfferingBlock,uint256 _harvestRefundingBlock) public onlyOwner{
    harvestOfferingBlock = _harvestOfferingBlock;
    harvestRefundingBlock = _harvestRefundingBlock;
  }
  function harvestAll() nonReentrant public  {
    require (block.number > endBlock, 'not harvest time');
    // require(block.number > finalWithdrawBlock,'not harvest time');
    syncUser(msg.sender);
    require (userInfo[msg.sender].amount > 0, 'have you participated?');
    ...
    user.offeringTokenAmount = offeringTokenAmount;
    user.refundingTokenAmount = refundingTokenAmount;
    if(!user.claimed&&offeringTokenAmount>0 && block.number > harvestOfferingBlock){
        user.claimed = true;
        user.amountDept = user.amountDept.add(offeringTokenAmount);
        TransferHelper.safeTransfer(offeringToken, address(msg.sender), offeringTokenAmount); 
        emit HarvestOffer(msg.sender, offeringTokenAmount);
    }

    if(!user.refundClaimed && refundingTokenAmount>0 &&block.number > harvestRefundingBlock ){
        user.refundClaimed = true;
          // lpToken.safeTransfer(address(msg.sender), refundingTokenAmount);
        TransferHelper.safeTransfer(lpToken, address(msg.sender), refundingTokenAmount);
        emit HarvestRefund(msg.sender, refundingTokenAmount);
  ...
  function hasHarvest(address _user) external view returns(bool) {
      require (userInfo[_user].synced,'must sync data first');
  ...
  function getUserAllocation(address _user) public view returns(uint256) {
    require (userInfo[_user].synced,'must sync data first');
    return userInfo[_user].amount.mul(1e12).div(dao.totalAmount()).div(1e6);
  }

  // get the amount of IFO token you will get
  function getOfferingAmount(address _user) public view returns(uint256) {
    require (userInfo[_user].synced,'must sync data first');
    if (dao.totalAmount() > raisingAmount) {
